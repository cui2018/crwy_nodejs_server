...
if ( HTTPS.status || SOCKETS.status ) {
	OPTIONS.key = FS.readFileSync("/usr/local/keys/gamev3.pay/214610233570367.key");
	OPTIONS.cert = FS.readFileSync("/usr/local/keys/gamev3.pay/public.pem");
	OPTIONS.ca = FS.readFileSync("/usr/local/keys/gamev3.pay/chain.pem");
}
if ( SERVER ) {
...
if ( HTTP.status ) {
	const APP = require("http").createServer( (req,res) => {
		...
		let originDomain = "http://www.edisonluorui.com";
		if ( req.headers.origin ) {
			let origin = req.headers.origin.split(":")[1];
			if (
				origin == "//www.edisonluorui.com" ||
				origin == "//crwy.edisonluorui.com" ||
				origin == "//fronts.edisonluorui.com"
